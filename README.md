...
| [aws_iam_policy.s3_env](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_policy) | resource |
...
| [aws_iam_policy_attachment.s3_env](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_policy_attachment) | resource |
...
| <a name="input_appspec_hook"></a> [appspec\_hook](#input\_appspec\_hook) | (Optional) enable lifecycle hook in codepipeline i.e. to trigger lambda after a lifecycle event | <pre>object({<br>    lifecycle_event_name = optional(string, "null")<br>    hooks_lambda_function_arn = optional(string, "null")<br>  })</pre> | `{}` | no |
...
| <a name="input_enable_appspec_hooks"></a> [enable\_appspec\_hooks](#input\_enable\_appspec\_hooks) | whether you want to enable lifecycle hooks in codepipeline i.e. to trigger lambda after a lifecycle event - https://docs.aws.amazon.com/codedeploy/latest/userguide/reference-appspec-file-structure-hooks.html | `bool` | `false` | no |
...
| <a name="input_enable_xray"></a> [enable\_xray](#input\_enable\_xray) | Enable X-Ray tracing for the task definition | `bool` | `false` | no |
| <a name="input_hooks_lambda_function_arn"></a> [hooks\_lambda\_function\_arn](#input\_hooks\_lambda\_function\_arn) | lambda function arn that you want to trigger after lifecycle event | `string` | `"null"` | no |
| <a name="input_lifecycle_event_name"></a> [lifecycle\_event\_name](#input\_lifecycle\_event\_name) | lifecycle event name from - BeforeInstall, AfterInstall, AfterAllowTestTraffic, BeforeAllowTraffic, AfterAllowTraffic | `string` | `"null"` | no |
| <a name="input_listener_rule_conditions"></a> [listener\_rule\_conditions](#input\_listener\_rule\_conditions) | List of conditions to use for the listener rule | <pre>object({ <br>              path_pattern = list(string) <br>              host_header = optional(list(string),[]), <br>              })</pre> | <pre>{<br>  "host_header": [],<br>  "path_pattern": [<br>    "/*"<br>  ]<br>}</pre> | no |
...
| <a name="input_side_car_resource_allocation_configuration"></a> [side\_car\_resource\_allocation\_configuration](#input\_side\_car\_resource\_allocation\_configuration) | Side car resource allocation configuration (Values given in decimal %) | `any` | <pre>{<br>  "app_config_agent": {<br>    "cpu": 0.125,<br>    "memory": 0.0625<br>  },<br>  "firelens": {<br>    "cpu": 0.125,<br>    "memory": 0.0625<br>  },<br>  "new_relic_infra_agent": {<br>    "cpu": 0.125,<br>    "memory": 0.125<br>  },<br>  "reverse_proxy": {<br>    "cpu": 0.125,<br>    "memory": 0.0625<br>  },<br>  "xray_daemon": {<br>    "cpu": 0.0625,<br>    "memory": 0.125<br>  }<br>}</pre> | no |
...
| <a name="input_volumes"></a> [volumes](#input\_volumes) | EFS Volumes to attach to the task | <pre>list(<br>    object({<br>      efs_volume_configuration = object({<br>        authorization_config = object({<br>          access_point_id = string,<br>        iam = string }),<br>        file_system_id     = string,<br>        root_directory     = string,<br>        transit_encryption = string,<br>      transit_encryption_port = number }),<br>      name = string<br>    })<br>  )</pre> | `[]` | no |
| <a name="input_vpc_name"></a> [vpc\_name](#input\_vpc\_name) | Name of the VPC to deploy to | `string` | n/a | yes |
| <a name="input_xray_daemon_image_uri"></a> [xray\_daemon\_image\_uri](#input\_xray\_daemon\_image\_uri) | Image URI for the X-Ray daemon container | `string` | `"public.ecr.aws/xray/aws-xray-daemon:latest"` | no |
